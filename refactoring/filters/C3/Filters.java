...
public class Filters {
    ...
    private static final BigDecimal ZERO_BIGDECIMAL = BigDecimal.ZERO;
    private static final BigInteger ZERO_BIGINTEGER = BigInteger.ZERO;
     ...
     */
    public static Class<?> promote(Class<?> firstType, Class<?> secondType) {
        if (firstType == secondType) {
            return unwrap(firstType);
        }

        Class<?> wrappedFirst = wrap(firstType);
        Class<?> wrappedSecond = wrap(secondType);
        if (wrappedFirst == wrappedSecond) {
            return unwrap(firstType);
        }

        boolean firstIsNumber = isNumber(wrappedFirst);
        boolean secondIsNumber = isNumber(wrappedSecond);
        if (firstIsNumber && secondIsNumber) {
            return promoteNumbers(wrappedFirst, wrappedSecond);
        }
        return promoteNonNumbers(firstType, wrappedFirst, firstIsNumber,
            secondType, wrappedSecond, secondIsNumber);
    }
    ...
    private static Class<?> promoteNumbers(Class<?> firstType, Class<?> secondType) {
        if (firstType == BigDecimal.class || secondType == BigDecimal.class) {
            return BigDecimal.class;
        }
        if (firstType == BigInteger.class || secondType == BigInteger.class) {
            Class<?> otherType = firstType == BigInteger.class ? secondType : firstType;
            return isFloatingPoint(otherType) ? BigDecimal.class : BigInteger.class;
        }
        if (firstType == Double.class || secondType == Double.class) {
            return double.class;
        }
        if (firstType == Float.class || secondType == Float.class) {
            return float.class;
        }
        if (firstType == Long.class || secondType == Long.class) {
            return long.class;
        }
        return int.class;
    }
    ...
    private static Class<?> promoteNonNumbers(Class<?> firstOriginal, Class<?> firstType,
        boolean firstIsNumber, Class<?> secondOriginal, Class<?> secondType, boolean secondIsNumber) {
        if (!firstIsNumber && secondIsNumber) {
            return promoteNumberWithOther(secondOriginal, secondType, firstType);
        }
        if (firstIsNumber && !secondIsNumber) {
            return promoteNumberWithOther(firstOriginal, firstType, secondType);
        }
        if (firstType == Character.class && secondType == String.class
            || firstType == String.class && secondType == Character.class) {
            return String.class;
        }
        return promoteReferenceTypes(firstOriginal, firstType, secondOriginal, secondType);
    }

    private static Class<?> promoteNumberWithOther(Class<?> numberOriginal,
        Class<?> numberType, Class<?> otherType) {
        if (otherType == Character.class || otherType == String.class) {
            return numberType == Byte.class || numberType == Short.class
                ? Integer.class : unwrap(numberOriginal);
        }
        return unwrap(numberOriginal);
    }

    private static Class<?> promoteReferenceTypes(Class<?> firstOriginal, Class<?> firstType,
        Class<?> secondOriginal, Class<?> secondType) {
        if (firstType == Object.class) {
            return unwrap(secondOriginal);
        }
        if (secondType == Object.class || firstType.isAssignableFrom(secondType)) {
            return unwrap(firstOriginal);
        }
        if (secondType.isAssignableFrom(firstType)) {
            return unwrap(secondOriginal);
        }

        boolean firstIsNonstandard = isNonstandardType(firstType);
        boolean secondIsNonstandard = isNonstandardType(secondType);
        if (firstIsNonstandard) {
            return secondIsNonstandard ? Object.class : unwrap(secondOriginal);
        }
        if (secondIsNonstandard) {
            return unwrap(firstOriginal);
        }
        return Object.class;
    }
    ...
    public static Object convert(Object o, Class<?> type, boolean strictNumericConversion) {
        ...
        if (!num) {
            if (type == String.class) {
                ...
                Object temporal = convertToTemporal(o, type);
                if (temporal != null) {
                    return temporal;
                }
    ...
    private static Object convertToTemporal(Object value, Class<?> targetType) {
    ...
    private static Object toLocalDate(Object value) {
        ...
        return value instanceof CharSequence ? LocalDate.parse((CharSequence) value) : null;
    }
    ...
    private static Object toLocalDateTime(Object value) {
        ...
        return value instanceof CharSequence ? LocalDateTime.parse((CharSequence) value) : null;
    }
    ...
    private static Object toLocalTime(Object value) {
        ...
        return value instanceof CharSequence ? LocalTime.parse((CharSequence) value) : null;
    }

    private static Object toOffsetTime(Object value) {
        if (value instanceof Time) {
        ...
        return value instanceof CharSequence ? OffsetTime.parse((CharSequence) value) : null;
    }

    private static Object toOffsetDateTime(Object value) {
        if (value instanceof Timestamp) {
            return ((Timestamp) value).toInstant().atZone(ZoneId.systemDefault()).toOffsetDateTime();
        }
        if (value instanceof Date) {
            return ((Date) value).toInstant().atZone(ZoneId.systemDefault()).toOffsetDateTime();
        }
        return value instanceof CharSequence ? OffsetDateTime.parse((CharSequence) value) : null;
    }
     ...
     */
    private static Exception unwrapPrivilegedException(Exception exception) {
        if (exception instanceof PrivilegedActionException) {
            return ((PrivilegedActionException) exception).getException();
        }
        return exception;
    }
    ...
    public static AggregateListener hintToAggregateListener(Object hint, ClassLoader loader) {
        ...
        if (hint instanceof String) {
            try {
                ...
                cause = unwrapPrivilegedException(e);
            }
        }
     ...
     */
    public static String clip(String value, String first, String last, boolean fail) {
        if (value == null) {
            return null;
        }
        boolean hasTerminals = value.length() >= first.length() + last.length()
            && value.startsWith(first) && value.endsWith(last);
        if (hasTerminals) {
            return value.substring(first.length(), value.length() - last.length()).trim();
        }
        if (fail) {
            throw new IllegalArgumentException(value + " is not valid escape syntax for JDBC");
        }
        return value;
    }
